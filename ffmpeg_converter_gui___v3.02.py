...
class FFmpegGUI(QMainWindow):
    ...
    def process_next(self):
        ...
        # File-Existenz-Check VOR dem Probe: wenn die Datei zwischen Batch-Start
        # und jetzt verschwunden ist (Netzwerk-Share weg, manuell verschoben),
        # sauber als Skip behandeln statt einen ffprobe auf einen toten Pfad
        # loszulassen.
        if not os.path.isfile(input_path):
            print(f"⚠️  Input file missing: {input_path} - skipping")
            self.batch_index += 1
            self.batch_skipped_count += 1
            if self.batch_index < len(self.batch_files) and not self.user_cancelled:
                self._schedule_next_batch_file(delay_ms=100)
            else:
                self._finalize_batch_after_skips()
            return

        # ---- Async-Probe mit Continuation ----
        #
        # Frueher (v30 original): setText() triggerte on_input_change, das
        # einen async ffprobe ohne Callback startete. process_next zog die
        # ganze Pipeline (audio-compat-Check, master-config, output-filename,
        # process_single_file → build_ffmpeg_command) SYNCHRON sofort danach
        # durch - mit STALE Stream-Daten der vorherigen Datei. Das war die
        # Wurzel des Magnum-S02E04-Bugs: build_ffmpeg_command sah den falschen
        # Subtitle-Stream-Index, FFmpeg crashte.
        #
        # Jetzt: blockSignals(True) verhindert den on_input_change-Trigger,
        # wir starten den ffprobe selber mit done_callback. Qt verbindet
        # _on_video_info_loaded ZUERST (in _load_video_info_async), unsere
        # Continuation DANACH - Qt feuert in Connect-Reihenfolge, also ist
        # der State (audio_streams, subtitle_streams, output_line, audio-
        # checkboxes, subtitle-combo) frisch wenn _process_next_after_probe
        # laeuft.
        ...
        self._load_video_info_async(
            input_path,
            done_callback=lambda info: self._process_next_after_probe(input_path, info)
        )

    def _process_next_after_probe(self, input_path, info):
        """
        Continuation von process_next - laeuft NACH _on_video_info_loaded
        wenn der ffprobe-Worker fertig ist. State ist hier garantiert frisch.

        Behandelt drei Faelle:
          1. User hat waehrend des Probes Cancel gedrueckt → still bailen
          2. ffprobe ist gescheitert → als regulaerer Batch-Fehler behandeln
          3. Output-Datei existiert bereits → Skip + naechstes File
          4. Sonst: regulaerer Encode-Run via process_single_file
        """
        # 1. Cancel- oder Reconfig-Pause-Check: kann waehrend der ~1-2s ffprobe-
        # Latenz ausgeloest worden sein. Wenn der User cancelled hat, hat
        # cancel_encoding bereits den Worker gestoppt und reset_ui_after_run
        # eingeplant - wir muessen hier einfach nur stillschweigend bailen.
        if self.user_cancelled or self.batch_paused_for_reconfig:
            return

        # 2. ffprobe-Fehler (kaputtes File, fehlende Codecs, IO-Error). _on_video_info_loaded
        # ist bei error/!is_valid frueh raus, ohne audio_streams/subtitle_streams/
        # output_line zu aktualisieren - hier weiterzumachen waere genau das alte
        # Stale-Daten-Problem in einer anderen Form. Stattdessen als regulaeren
        # Batch-Fehler ueber ffmpeg_finished melden, das uebernimmt Index-Increment
        # + Folge-Scheduling.
        if info.error or not info.is_valid:
            msg = f"ffprobe failed: {info.error or 'file unreadable / no valid streams'}"
            print(f"⚠️  {msg}")
            self.ffmpeg_finished(-1, msg)
            return

        # ---- Ab hier: State ist frisch, normale Pipeline ----
        ...
        # 3. Wenn die Output-Datei bereits existiert, wird sie übersprungen statt
    ...
    def _schedule_next_batch_file(self, delay_ms: int = 500):
        """
        Tracked-QTimer-Helper fuer den Aufruf von process_next nach kurzer
        Pause. Vorher in process_next + ffmpeg_finished dupliziert; jetzt
        zentral, damit das Timer-Tracking (kein Memory-Leak via
        pending_batch_timers) an einer Stelle gepflegt wird.
        """
        timer = QTimer()
    ...
    def ffmpeg_finished(self, return_code: int, output: str):
        ...
        if self.batch_mode:
            ...
            if self.batch_index < len(self.batch_files) and not self.user_cancelled and not self.batch_paused_for_reconfig:
                self._schedule_next_batch_file(delay_ms=500)
