...
Highlights:
- Hardware Encoding: Multi-vendor GPU support
  * NVIDIA NVENC: AV1, H.265, H.264 (-init_hw_device cuda:N)
  * AMD AMF:      AV1, H.265, H.264 (-init_hw_device d3d11va:N)
  * Intel QSV:    AV1, H.265, H.264 (-init_hw_device qsv:N)
  * Software:     SVT-AV1, x265, x264, libvpx-vp9
...
v3.0 changes: AMD AMF and Intel QSV hardware encoder support added.
GPU detection now multi-vendor (nvidia-smi + Windows WMI / Linux lspci).
Encoder dropdown filters by selected GPU's vendor — only that vendor's
hardware codecs are listed alongside software encoders. NVENC-specific
quality tunings (preset p7, tune uhq, multipass fullres, rc-lookahead)
remain NVENC-only; AMF/QSV use their respective driver defaults.
Mediathek-Safe profile/level settings extended to AMF/QSV with the
correct level-format per encoder (4.1 vs 41).
...
Uses FFmpeg 8.0 syntax (`-init_hw_device <backend>:N`, `-fps_mode cfr`).

Acknowledgements:
FFmpeg, PySide6, psutil, Python, NVIDIA NVENC, AMD AMF, Intel Quick Sync,
SVT-AV1, x264, x265, and Claude (Anthropic) for AI-coding support on
architecture, refactoring and bug fixes.
...
# ----------------------------------------------------------------------------
#
# Each vendor uses its own enumeration tool, and FFmpeg addresses devices
# per backend (cuda:N for NVENC, d3d11va:N for AMF, qsv:N for QSV). Therefore
# the per-GPU 'index' returned here is per-vendor (each vendor counts from 0),
# not a global enumeration.
#
# get_gpu_info() returns a list of dicts:
#   {'index': N, 'name': str, 'vendor': 'nvidia'|'amd'|'intel'}
#
# Detection chain:
#   - NVIDIA: nvidia-smi -L (existing, just normalized to the new schema)
#   - AMD/Intel on Windows: WMI via wmic (fallback PowerShell Get-CimInstance)
#   - AMD/Intel on Linux:   lspci -mm (filtered for VGA/3D/Display class)
#   - macOS:                returns [] (videotoolbox would need different
#                           command-line wiring; out of scope for v3.0)
#
# All subprocess calls are guarded by short timeouts (5-10s) and broad
# except clauses, so a missing/broken tool just yields no entries instead
# of crashing the GUI.
# ----------------------------------------------------------------------------

# Suppress console-window pop-up on Windows when calling subprocesses
_SUBPROC_FLAGS = (
...
def _detect_nvidia_gpus():
    ...
    FIX (v2.6): Frueher wurde line.split(': ') verwendet - das splittet aber
    auch am ': ' innerhalb von '(UUID: GPU-...)', wodurch der Name als
    ...
    try:
        ...
        for line in out.strip().splitlines():
            ...
            if gpu_name:
                gpus.append({'index': gpu_index, 'name': gpu_name, 'vendor': 'nvidia'})
...
def _detect_windows_amd_intel():
    """AMD + Intel via Windows WMI.

    Tries `wmic` first (fast, ~200ms), falls back to PowerShell's
    Get-CimInstance (slower, ~1-2s) since wmic is deprecated since
    Windows 10 21H1 and may be removed entirely in future builds.
    """
    names = []
    try:
        out = subprocess.check_output(
    ...
    return _classify_gpu_names(names)


def _detect_linux_amd_intel():
    """AMD + Intel via lspci -mm (machine-readable format).

    Filters for VGA/3D/Display device classes and skips NVIDIA (handled
    separately by nvidia-smi).
    """
    try:
        out = subprocess.check_output(
            ['lspci', '-mm'],
    ...
    names = []
    for line in out.splitlines():
        # Format: BUS_ID "Class" "Vendor" "Device" ...
        # Only display-related classes
        if not any(k in line for k in ('VGA', '3D', 'Display')):
            continue
        # Quoted fields: parts[1::2] gives every odd-position element
        parts = [p.strip('"') for p in line.split('"')[1::2]]
        if len(parts) >= 3:
            # parts: [class, vendor, device, ...]
...
def _classify_gpu_names(names):
    """Klassifiziert eine Liste von GPU-Namen nach Vendor (AMD/Intel) und
    weist pro Vendor einen 0-basierten Index zu. NVIDIA wird hier bewusst
    ignoriert - die werden separat ueber nvidia-smi erkannt, was zuverlaes-
    siger ist als Namens-Matching (z.B. fuer Datacenter-Karten ohne 'NVIDIA'
    im Display-Namen).
    """
    gpus = []
    amd_idx = 0
    ...
    for name in names:
        nl = name.lower()
        if 'nvidia' in nl:
            continue  # Wird ueber nvidia-smi erfasst
        if 'amd' in nl or 'radeon' in nl or 'ati ' in nl or 'ati[' in nl:
            gpus.append({'index': amd_idx, 'name': name, 'vendor': 'amd'})
            amd_idx += 1
        elif 'intel' in nl:
            gpus.append({'index': intel_idx, 'name': name, 'vendor': 'intel'})
            intel_idx += 1
        # Alles andere (Microsoft Basic Display, VirtualBox, etc.) ignorieren
    return gpus
...
def get_gpu_info():
    ...
    The index is per-vendor (each vendor counts from 0). This matches how
    FFmpeg addresses devices on the command line:
        - NVENC: -init_hw_device cuda:N
        - AMF:   -init_hw_device d3d11va:N
        - QSV:   -init_hw_device qsv:N

    Returns an empty list if no supported GPUs are found, or if the
    detection tools are unavailable (e.g. macOS, headless Linux).
    """
    gpus = []
    gpus.extend(_detect_nvidia_gpus())

    if sys.platform == 'win32':
        gpus.extend(_detect_windows_amd_intel())
    elif sys.platform.startswith('linux'):
        gpus.extend(_detect_linux_amd_intel())
    # macOS: out of scope - videotoolbox wiring would need its own command
    # path (no -init_hw_device), so we deliberately don't surface it here.
...
class VideoInfoWorker(QRunnable):
    ...
    def _get_duration_safe(self) -> float:
        """Sichere Duration-Extraktion mit Timeout und Fallback-Strategie.

        Versucht zuerst die Container-Duration (`format=duration`) - das ist
        der Standardpfad und liefert bei modernen Containern (MKV, MP4, MOV)
        zuverlaessig die Gesamtlaenge.

        Wenn das 'N/A' zurueckgibt - typisch bei alten MPEG-TS-Aufnahmen,
        Live-Captures, .vob-DVD-Rips oder Files mit kaputtem Container-
        Header - wird auf die Video-Stream-Duration (`stream=duration`)
        zurueckgefallen. Das funktioniert bei vielen aelteren Files, weil
        dort die Stream-Header die Duration tragen, aber der Container-
        Header sie nicht.

        Gibt 0.0 zurueck, wenn beide Pfade fehlschlagen. Das Programm
        funktioniert dann weiter, nur die Progress-Anzeige ist unvollstaendig.
        """
        # 1. Versuch: Container-Header (schnell)
        duration = self._probe_duration_field('format=duration', select_video=False)
        if duration > 0:
            return duration

        # 2. Versuch: Video-Stream-Duration (Fallback fuer MPEG-TS/alte Files)
        duration = self._probe_duration_field('stream=duration', select_video=True)
        if duration > 0:
            fname = os.path.basename(self.filepath)
            print(f"ℹ️ Container ohne Duration-Header - nutze Stream-Duration für '{fname}'")
            return duration

        # Beide Pfade fehlgeschlagen
        fname = os.path.basename(self.filepath)
        print(f"⚠️ Duration nicht ermittelbar für '{fname}' "
              f"(Progress-Anzeige wird unvollständig sein - Encoding läuft trotzdem)")
        return 0.0

    def _probe_duration_field(self, entry: str, select_video: bool = False) -> float:
        """Hilfsmethode: probt ein Duration-Feld via ffprobe.

        Gibt 0.0 zurueck bei Timeout, ffprobe-Fehler, leerer Antwort,
        'N/A', oder unparsbaren Werten. Das ist by design - der Aufrufer
        nutzt den Returnwert als Truthy-Test fuer 'erfolgreich/nicht'.
        """
        cmd = ["ffprobe", "-v", "error"]
        if select_video:
        ...
        try:
            ...
            # Multi-Stream-Schutz: erste nicht-leere Zeile nehmen.
            # ffprobe kann bei multiple Streams mehrzeilig antworten.
            first_line = next((l.strip() for l in out.splitlines() if l.strip()), '')
            if not first_line or first_line.upper() == 'N/A':
                return 0.0
    ...
    def _get_fps_safe(self) -> float:
        ...
        try:
            ...
            # Multi-Stream-Schutz: erste nicht-leere Zeile.
            out = result.stdout.strip()
            fps_str = next((l.strip() for l in out.splitlines() if l.strip()), '')
            # 'N/A' (alte Files ohne Frame-Rate-Info) als 0 behandeln
    ...
    def _get_resolution_safe(self) -> Tuple[int, int]:
        ...
        Robustes Parsing fuer Edge-Cases:
        - Mehrere Video-Streams (mehrzeilige Antwort wie '1920,1080\\n1920,1080')
          -> nur die erste Zeile wird verwendet
        - Trailing-Komma oder zusaetzliche Felder ('1920,1080,extra')
          -> nur die ersten zwei Werte werden verwendet
        - 'N/A' / leere Werte fuer Width oder Height
          -> graceful Fallback auf 0 fuer den jeweiligen Wert
        """
        try:
            result = subprocess.run(
                ["ffprobe", "-v", "error", "-select_streams", "v:0",
            ...
            if not first_line or ',' not in first_line:
                return 0, 0
            # Nur die ersten zwei Komma-getrennten Werte verwenden -
            # falls ffprobe ein drittes Feld anhaengt oder ein Trailing-
            # Komma einbaut, gibt das vorher 'too many values to unpack'.
...
class FFmpegWorker(QThread):
    ...
    # progress_signal: dritter Parameter ist total_size_bytes des Encodes -
    # bei Files > 2 GB sprengt das den int32, den Qt's `int` Signal-Typ
    # darstellt (libshiboken wirft RuntimeWarning + OverflowError beim emit).
    # `object` umgeht die Qt-Konversion vollstaendig: die Python-int wird
    # 1:1 an den Slot weitergereicht, der sie eh nur als Zahl in Arithmetik
    # benutzt (predicted_file_size_mb).
    progress_signal = Signal(float, float, object)
    ...
    @staticmethod
    def _to_signed_int32(rc):
        """Konvertiert Windows-DWORDs (uint32) zu signed int32.

        Hintergrund: Auf Windows liefert subprocess.process.returncode
        rohe DWORDs - z.B. 0xB1A2D8AB (= 2981409195) bei einem QSV-Init-
        Fehler. Qt's `Signal(int, str)` mappt 'int' aber auf signed
        32-bit, und libshiboken wirft OverflowError bei Werten > 2^31.

        Werte > 0x7FFFFFFF werden zu ihrem signed-32-bit-Aequivalent
        konvertiert (z.B. 2981409195 -> -1313558101, was dem urspruenglichen
        Windows-NTSTATUS entspricht). Werte im normalen Range (0..2^31-1)
        bleiben unveraendert. None wird zu -1 normalisiert.
        """
        if rc is None:
            return -1
        if rc > 0x7FFFFFFF:
    ...
    def run(self):
        ...
        try:
            ...
            # FIX (v3.0): Auf Windows liefert subprocess.returncode einen
            # DWORD (uint32). Bei NTSTATUS-Werten mit Bit 31 gesetzt -
            # z.B. QSV-Init-Fehler wie 0xB1A2D8AB (= 2981409195 unsigned
            # = -1313558101 signed) - ist der Wert > 2^31 und passt nicht
            # in Qt's int Signal-Typ (signed 32-bit). Ohne Konversion
            # wirft libshiboken 'OverflowError' und der Slot wird gar
            # nicht erst aufgerufen, sodass der Fehler-Dialog die
            # Encoding-Pipeline nicht stoppen kann.
            return_code = self._to_signed_int32(return_code)
...
class RemuxDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        #
        # Hintergrund (v3.0.4): Bei Files mit vielen Audio-Tracks und
        # Subtitles wuchs der Dialog ueber die Bildschirmgroesse hinaus,
        # sodass die Action-Buttons nicht mehr sichtbar waren. Loesung:
        # nur die scrollbare Mitte darf wachsen; Header (zur Orientierung)
        # und Buttons (zum Bedienen) bleiben immer im sichtbaren Bereich.
...
class FFmpegGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("FFmpeg Converter GUI 3.0.2")
        ...
        # v3.0.3: Failed-Counter getrennt von Skipped. Skipped = bewusst
        # uebersprungen (Output existiert, Subtitle inkompatibel etc.);
        # Failed = FFmpeg ist mit Returncode != 0 zurueckgekommen. Beide
        # gehen in die Toast-Bilanz, aber unter ihrem eigenen Label.
        self.batch_failed_count = 0

        # Flag fuer einmaligen Subtitle-Retry pro File: verhindert
        # Endlosschleife wenn der Retry-Run selbst wieder mit einem
        # Subtitle-Fehler zurueckkommt. Siehe ffmpeg_finished().
    ...
    CONTAINER_VIDEO_CODECS = {
        ...
        # Pro Container alle drei Hardware-Vendors + Software. Der GPU-Filter
        # in update_video_codec_options() blendet die nicht passenden Vendor-
        # Encoder spaeter aus (z.B. *_amf nur bei AMD-GPU-Auswahl).
        ...
        'mov':  ['hevc_nvenc', 'h264_nvenc',
                 'hevc_amf',   'h264_amf',
                 'hevc_qsv',   'h264_qsv',
                 'libx265', 'libx264'],
        'webm': ['av1_nvenc', 'av1_amf', 'av1_qsv',
                 'libsvt-av1', 'libvpx-vp9'],
    }
    ...
    VIDEO_CODEC_LABELS = {
        ...
        # AMD AMF
        'av1_amf':    'AV1 (av1_amf)',
        'hevc_amf':   'H.265 (hevc_amf)',
        'h264_amf':   'H.264 (h264_amf)',
        # Intel Quick Sync
        'av1_qsv':    'AV1 (av1_qsv)',
        'hevc_qsv':   'H.265 (hevc_qsv)',
        'h264_qsv':   'H.264 (h264_qsv)',
        # Software
    ...
    # Vendor-spezifische Encoder-Sets - zentral definiert, an einer Stelle
    # pflegbar. Wird in update_video_codec_options(), build_ffmpeg_command(),
    # _on_container_changed() etc. genutzt.
    NVENC_CODECS = ('av1_nvenc', 'hevc_nvenc', 'h264_nvenc')
    AMF_CODECS   = ('av1_amf',   'hevc_amf',   'h264_amf')
    QSV_CODECS   = ('av1_qsv',   'hevc_qsv',   'h264_qsv')
    GPU_CODECS   = NVENC_CODECS + AMF_CODECS + QSV_CODECS

    # Subtitle-Klassifikation (gleiche Werte wie RemuxDialog)
    # Text-basiert: koennen via -c:s copy in beliebige Container, sind
    # auch fuer Burn-in (libass) geeignet.
    # Picture-basiert: koennen NUR via -c:s copy in MKV/MOV uebernommen
    # werden, aber Burn-in geht nicht. Bei manchen Files mit kaputten
    # Codec-Metadaten scheitert auch -c:s copy - der Matroska-Muxer faellt
    # dann auf seinen text-basierten Default-Encoder (SSA) zurueck und
    # crasht mit "Subtitle encoding currently only possible from text to
    # text or bitmap to bitmap".
    SUB_CONVERTIBLE = {'subrip', 'srt', 'ass', 'ssa', 'mov_text', 'webvtt'}
    SUB_PICTURE_BASED = {'hdmv_pgs_subtitle', 'pgs', 'dvd_subtitle',
                         'dvb_subtitle', 'xsub'}

    # Vendor → Codec-Set Lookup (fuer Filter-Logik bei GPU-Wechsel)
    VENDOR_CODECS = {
        'nvidia': NVENC_CODECS,
        'amd':    AMF_CODECS,
        'intel':  QSV_CODECS,
    ...
    # ------------------------------------------------------------------
    # GPU Capability Matrix (v3.0.2)
    # ------------------------------------------------------------------
    # Welche Codec-Familien KANN die jeweilige GPU in Hardware encodieren?
    # Quellen (Stand 2026-Q1):
    #   - NVIDIA: developer.nvidia.com NVENC Application Note
    #   - Intel:  intel.com onevpl Media Capabilities + Wikipedia QSV
    #   - AMD:    Wikipedia VCN + AMF GitHub
    #
    # Logik: Pattern-Match auf den Display-Namen der GPU. Konservativer
    # Default: bei unbekannten GPU-Namen eines bekannten Vendors gehen wir
    # von HEVC+H.264 aus - das funktioniert seit ~2015 auf allen relevanten
    # Hardware-Generationen (Maxwell-2 / Skylake / Polaris). AV1 ist ein
    # echtes Differenzierungs-Feature der 2022er-Generationen, das wir nur
    # bei explizit erkannten Modellen anbieten.
    #
    # Spezialfaelle ohne Hardware-Encoder (kein AMF/NVENC trotz GPU):
    #   - NVIDIA GT 1030, GP108-MX-Mobile (z.B. MX450)
    #   - AMD RX 6400, RX 6500 (Navi 24 mit deaktivierter VCN-Einheit)
    # Diese geben eine leere Capability-Menge zurueck, sodass der GUI auf
    # Software-Encoding zurueckfaellt.
    # ------------------------------------------------------------------

    @staticmethod
    def _codec_family(codec):
        """Maps codec_id -> family ('av1' | 'hevc' | 'h264' | 'vp9')."""
        if codec.startswith('av1') or codec == 'libsvt-av1':
            return 'av1'
    ...
    @staticmethod
    def _gpu_codec_families(vendor, gpu_name):
        """Returns set of codec families this GPU can encode in hardware.

        Returns empty set for GPUs known to lack a hardware encoder
        entirely (e.g. NVIDIA GT 1030, AMD RX 6400/6500). Returns the
        conservative default {'hevc', 'h264'} for unknown models, since
        these two families have been universally supported on the
        relevant hardware generations since ~2015.
        """
        n = (gpu_name or '').upper()

        if vendor == 'nvidia':
            # GPUs ohne Encoder
            if re.search(r'\bGT\s*1030\b', n):
                return set()
            if re.search(r'\bMX\s*4[5-7]0\b', n):
                # MX450/460/470 (GP108-Derivate, kein NVENC)
                return set()
            # AV1: Ada Lovelace (RTX 40), Blackwell (RTX 50), zukuenftig RTX 60+
            if re.search(r'\bRTX\s*[4-9]\d{3}\b', n):
                return {'av1', 'hevc', 'h264'}
            # Datacenter Ada (L40, L4, L40S) - alle koennen AV1
            if re.search(r'\bL\s*(4|40|40S)\b', n):
                return {'av1', 'hevc', 'h264'}
            # Default: HEVC + H.264
            return {'hevc', 'h264'}

        if vendor == 'amd':
            # RX 6400 / 6500 haben keinen Encoder (Navi 24 mit deaktivierter VCN)
            if re.search(r'\bRX\s*6[45]00\b', n):
                return set()
            # AV1: RDNA 3 (RX 7xxx), RDNA 4 (RX 9xxx)
            if re.search(r'\bRX\s*[79]\d{3}\b', n):
                return {'av1', 'hevc', 'h264'}
            # Default: HEVC + H.264 (Polaris RX 4xx und neuer)
            return {'hevc', 'h264'}

        if vendor == 'intel':
            # AV1: Arc A-Serie (Alchemist) und Arc B-Serie (Battlemage).
            # UHD 7xx / Iris Xe sind Gen12 XeLP - koennen AV1 nur dekodieren,
            # NICHT encodieren. Daher matchen wir explizit nur "Arc Axxx" /
            # "Arc Bxxx" und nicht alle Iris-Xe-Varianten.
            if re.search(r'\bARC\b.*\b[AB]\d{3}\b', n):
                return {'av1', 'hevc', 'h264'}
            # Default: HEVC + H.264 (Skylake UHD 6xx und neuer)
            return {'hevc', 'h264'}

        # Unbekannter Vendor (sollte nicht passieren) - sicherer Default
        return {'hevc', 'h264'}
    ...
    def _create_video_codec_and_resolution_layout(self):
        ...
        self.mediathek_safe_checkbox.setToolTip(
            ...
            "• Hardware encoders (NVENC/AMF/QSV): Profile/Level + GOP tuning\n"
            "\n"
        ...
        self.force_av1_checkbox.setToolTip(
            ...
            "\n"
            "Use at your own risk - H.265 (any vendor) recommended instead!"
        )
    ...
    def _update_force_av1_visibility(self):
        ...
        - Irgendein AV1-Encoder ausgewählt (NVENC/AMF/QSV/SVT) UND
        - Mediathek-Safe aktiviert

        v3.0: Force-AV1-Checkbox erscheint auch bei AMD- und Intel-AV1, weil
        das zugrundeliegende Black-Screen-Problem ein Decoder-Problem auf
        der Mediathek-Player-Seite ist - unabhaengig vom Encoder-Vendor.
        """
        video_codec = self.video_codec_combo.currentData() or ''
        is_av1 = 'av1' in video_codec
    ...
    def _create_gpu_selection_layout(self):
        ...
        if self.gpu_info_list:
            for gpu in self.gpu_info_list:
                # userData ist ab v3.0 ein dict {'index': N, 'vendor': str},
                # nicht mehr nur ein int. build_ffmpeg_command(),
                # update_video_codec_options() etc. lesen 'vendor' raus.
                self.gpu_combo.addItem(
                    gpu['name'],
                    userData={'index': gpu['index'], 'vendor': gpu['vendor']},
                )
        self.gpu_combo.addItem("CPU", userData=None)

        # FIX: Combo automatisch an laengsten Eintrag anpassen, damit Namen
        # wie "Intel(R) Arc(TM) A770 Graphics" (30 Zeichen) oder
        # "AMD Radeon RX 7900 XTX" (22) und "NVIDIA GeForce RTX 5090" (23)
        # nicht vom Dropdown-Pfeil abgeschnitten werden. v3.0: 30 statt 24.
        self._configure_combo_dpi_aware(
            self.gpu_combo, visible_chars=30, popup_extra_chars=4
        )
    ...
    def update_advanced_options_state(self):
        # Die NVENC-Quality-Tunings (preset p7, tune uhq, multipass fullres,
        # rc-lookahead) sind explizit NVENC-only. Bei AMD-AV1 oder Intel-AV1
        # bleiben die Checkboxen disabled - der User kann sie nicht
        # aktivieren, und build_ffmpeg_command() ignoriert sie ohnehin
        # ueber den is_nvenc-Branch.
        gpu_data = self.gpu_combo.currentData()
        is_gpu = gpu_data is not None
        vendor = gpu_data['vendor'] if is_gpu else None
        is_av1_nvenc = (vendor == 'nvidia'
                        and self.video_codec_combo.currentData() == 'av1_nvenc')

        self.preset_checkbox.setEnabled(is_av1_nvenc)
        self.tune_checkbox.setEnabled(is_av1_nvenc)
        self.multipass_checkbox.setEnabled(is_av1_nvenc)
        self.rc_lookahead_checkbox.setEnabled(is_av1_nvenc)
        self.rc_lookahead_line.setEnabled(is_av1_nvenc and self.rc_lookahead_checkbox.isChecked())
    ...
    def update_video_codec_options(self):
        ...
        Container-Wahl, GPU-Vendor UND GPU-Capability.

        v3.0: Pro GPU-Vendor (NVIDIA / AMD / Intel) wird nur die jeweilige
        Hardware-Encoder-Familie angeboten - eine NVIDIA-GPU listet nur
        NVENC, eine AMD-GPU nur AMF, eine Intel-GPU nur QSV. Software-
        Encoder sind ueberall verfuegbar.

        v3.0.2: Zusaetzlich werden Hardware-Encoder pro GPU-Modell gefiltert.
        AV1 wird z.B. nur bei NVIDIA RTX 40+, Intel Arc A/B-Series und
        AMD RX 7000+ angeboten. Eine Intel UHD Graphics 770 sieht
        beispielsweise nur HEVC_QSV und H.264_QSV - nicht mehr AV1_QSV
        (was die GPU physisch nicht kann und in einen FFmpeg-Crash laeuft).
        GPUs ganz ohne Encoder (RX 6400/6500, GT 1030) zeigen nur Software-
        Encoder mit einer Console-Warnung.

        - GPU verfuegbar: nur die GPU-faehigen Codec-Familien, dann Software
        - CPU gewaehlt:   nur Software-Encoder, AV1-CPU disabled (zu langsam)
        ...
        if is_gpu:
            # Vendor-spezifische Hardware-Encoder
            vendor_codecs = self.VENDOR_CODECS.get(vendor, ())

            # Capability-Filter: welche Codec-Familien KANN diese spezifische
            # GPU? (Eine UHD 770 kann hevc+h264, aber kein av1.)
            gpu_name = self.gpu_combo.currentText()
            capable_families = self._gpu_codec_families(vendor, gpu_name)

            if not capable_families:
                # GPU ohne Hardware-Encoder (z.B. RX 6400/6500, GT 1030).
                # Wir loggen das einmal und fallen still auf Software zurueck.
                # Die User-Auswahl steht im Encoder-Dropdown weiterhin auf
                # der GPU - der Codec-Combo enthaelt aber keine HW-Encoder.
                if not getattr(self, '_no_encoder_warned', None) == gpu_name:
                    print(f"⚠️ '{gpu_name}' has no hardware video encoder - "
                          f"only software encoders will be available")
                    self._no_encoder_warned = gpu_name
                capable_codecs = []
            else:
                capable_codecs = [
                    c for c in vendor_codecs
                    if self._codec_family(c) in capable_families
                ]

            # Hardware-Encoder zuerst, dann Software.
            ordered = [c for c in allowed_codecs if c in capable_codecs]
            ordered += [c for c in allowed_codecs if c not in self.GPU_CODECS]
        else:
    ...
    def _on_container_changed(self):
        ...
        if video_incompatible or audio_incompatible:
            ...
            if video_incompatible:
                ...
                # Suggested replacement: erster verfuegbarer Codec im neuen
                # Container, gefiltert nach aktuellem GPU-Vendor.
                gpu_data = self.gpu_combo.currentData()
                is_gpu = gpu_data is not None
                vendor = gpu_data['vendor'] if is_gpu else None
                if is_gpu:
                    vendor_codecs = self.VENDOR_CODECS.get(vendor, ())
                    suggested_v = next(
                        (c for c in allowed_video if c in vendor_codecs),
                        allowed_video[0]
                    )
                else:
                    suggested_v = next(
                        (c for c in allowed_video if c not in self.GPU_CODECS),
                        allowed_video[0]
                    )
    ...
    def build_ffmpeg_command(self, input_path, output_path, log=False):
        ...
        is_gpu = gpu_data is not None
        vendor = gpu_data['vendor'] if is_gpu else None
        hw_index = gpu_data['index'] if is_gpu else None

        is_nvenc = is_gpu and video_codec in self.NVENC_CODECS
        is_amf   = is_gpu and video_codec in self.AMF_CODECS
        is_qsv   = is_gpu and video_codec in self.QSV_CODECS
        ...
        # Vendor-spezifische Hardware-Device-Initialisierung VOR -i.
        # Indices sind per-Vendor (cuda:N nummeriert NVIDIA-GPUs, d3d11va:N
        # AMD-GPUs - jeweils ab 0). Das matcht get_gpu_info().
        # Alt (entfernt): codec-lokales `-gpu N` (deprecated, in Multi-GPU
        # nicht zuverlaessig wirksam).
        if is_nvenc:
            cmd.extend(["-init_hw_device", f"cuda:{hw_index}"])
        elif is_amf:
            # AMF nutzt D3D11 als HW-Backend auf Windows. Damit der Encoder
            # weiss, welche AMD-GPU er bei Multi-GPU-Setups (z.B. iGPU+dGPU)
            # waehlen soll, binden wir das Device an einen Alias und
            # referenzieren ihn per -filter_hw_device.
            cmd.extend([
                "-init_hw_device", f"d3d11va=amf_dev:{hw_index}",
                "-filter_hw_device", "amf_dev",
            ])
        elif is_qsv:
            # QSV-Init ist auf Intel-Treibern haerter zu erwischen als
            # NVENC/AMF. Wir nutzen zwei Pfade:
            #
            # Index 0 (Single-Intel-Setup, ~95% der Faelle): einfaches
            #   qsv=qsv_dev:hw_any - 'hw_any' lasst FFmpeg die geeignete
            #   MFX-Implementation automatisch waehlen. Robust und auf
            #   Intel UHD Graphics 770 + Arc verifiziert.
            #
            # Index > 0 (Multi-Intel, sehr selten - z.B. iGPU + Arc dGPU
            #   im selben System): wir bauen ein D3D11VA-Parent-Device
            #   mit explizitem Adapter-Index und referenzieren es per @
            #   im QSV-Init. Das ist die einzige saubere Multi-Intel-
            #   Adressierung auf Windows.
            #
            # FRUEHER (entfernt): qsv=qsv_dev:hw_any,child_device={N}
            # Das war syntaktisch falsch - ':hw_any' ist ein Implementation-
            # Selector, 'child_device=N' will einen Device-Pfad. Die
            # Kombination warf 'MFX session: -9' / Windows-NTSTATUS auf
            # Intel UHD Graphics 770.
            if hw_index == 0:
                cmd.extend([
                    "-init_hw_device", "qsv=qsv_dev:hw_any",
                    "-filter_hw_device", "qsv_dev",
                ])
            else:
                cmd.extend([
                    "-init_hw_device", f"d3d11va=qsv_dx:{hw_index}",
                    "-init_hw_device", "qsv=qsv_dev@qsv_dx",
                    "-filter_hw_device", "qsv_dev",
                ])
        ...
        if is_gpu:
            ...
            # NVENC-spezifische Quality-Tunings (preset p7, tune uhq,
            # multipass fullres, rc-lookahead) gibt es genau bei AV1_NVENC.
            # Die Optionsnamen und -werte sind nicht 1:1 auf AMF/QSV
            # uebertragbar - dort uebernehmen die Encoder ihre Treiber-
            # Defaults. Die UI-Checkboxen sind in update_advanced_options_state()
            # entsprechend disabled, dieser Block schuetzt zusaetzlich gegen
            # Race-Conditions / settings-restore.
        ...
        if self.mediathek_safe_checkbox.isChecked():
            ...
            # Hardware-Encoder bekommen zusaetzlich GOP- und Profile/Level-
            # Tuning fuer maximale Player-Kompatibilitaet. Software-Encoder
            # (libx264/265) haben gute Defaults und bleiben unangetastet.
            if is_nvenc or is_amf or is_qsv:
                codecs.extend(["-g", "48"])            # Keyframe alle 48 Frames (~2 Sek bei 24fps)

                # `-strict_gop` ist eine NVENC-only Option (FFmpeg-Encoder-Flag).
                # AMF/QSV haben kein direktes Pendant und ignorieren es nicht
                # einfach, sondern werfen einen Fehler - daher NVENC-only.
                if is_nvenc:
                    codecs.extend(["-strict_gop", "1"])

                # Codec-spezifische Profile/Level fuer max. Kompatibilitaet.
                # Wichtig: NVENC/AMF nutzen das humanlesbare Level-Format
                # ("4.1"), QSV nutzt das numerische ("41" = 4*10+1).
                if video_codec == 'h264_nvenc':
                    codecs.extend(["-profile:v", "high", "-level", "4.1"])
                    if log:
                        print("📺 Mediathek-Safe: H.264 NVENC (profile=high, level=4.1, strict_gop)")
                elif video_codec == 'hevc_nvenc':
                    codecs.extend(["-profile:v", "main", "-level", "4.1"])
                    if log:
                        print("📺 Mediathek-Safe: HEVC NVENC (profile=main, level=4.1, strict_gop)")
                elif video_codec == 'h264_amf':
                    codecs.extend(["-profile:v", "high", "-level", "4.1"])
                    if log:
                        print("📺 Mediathek-Safe: H.264 AMF (profile=high, level=4.1)")
                elif video_codec == 'hevc_amf':
                    codecs.extend(["-profile:v", "main", "-level", "4.1"])
                    if log:
                        print("📺 Mediathek-Safe: HEVC AMF (profile=main, level=4.1)")
                elif video_codec == 'h264_qsv':
                    codecs.extend(["-profile:v", "high", "-level", "41"])
                    if log:
                        print("📺 Mediathek-Safe: H.264 QSV (profile=high, level=41)")
                elif video_codec == 'hevc_qsv':
                    codecs.extend(["-profile:v", "main", "-level", "41"])
                    if log:
                        print("📺 Mediathek-Safe: HEVC QSV (profile=main, level=41)")
                elif 'av1' in video_codec:
                    # AV1 (alle Vendors): kein profile/level - braucht's nicht.
                    # Nur g=48 ist gesetzt. Force-AV1 fuegt Color-Space-Hints
                    # hinzu - ebenfalls vendor-unabhaengig (es sind generische
                    # FFmpeg-Output-Flags, kein Encoder-spezifisches Tuning).
                    if self.force_av1_checkbox.isChecked():
                        ...
                        if log:
                            print(f"📺 Mediathek-Safe: {video_codec} FORCE MODE (g=48, bt709 color space)")
                    else:
                        if log:
                            print(f"📺 Mediathek-Safe: {video_codec} (g=48)")
            else:
                if log:
        ...
        # Audio-Stream-Selection mit Stale-Daten-Schutz.
        #
        # Im Batch-Modus startet `process_next` per `setText()` einen async ffprobe
        # und ruft `process_single_file` synchron sofort danach auf. Wenn der Probe
        # fuer das aktuelle File noch nicht durch ist, halten audio_track_checkboxes
        # noch die Stream-Daten der VORHERIGEN Datei. Hat die neue Datei weniger
        # Audio-Tracks, kracht FFmpeg hart mit:
        #   "Failed to set value '0:N' for option 'map': Invalid argument"
        #
        # Schichtweise Absicherung:
        #   1. Filter: nur Streams mappen, deren Index in den aktuellen
        #      self.audio_streams existiert (verhindert harte Map-Fehler bei
        #      strukturell unterschiedlichen Files)
        #   2. Fallback: wenn nichts uebrig bleibt (alle Checkbox-Streams stale),
        #      nimm den ersten Audio-Stream der aktuellen Source
        #   3. Optional-Modifier `?`: selbst wenn ein Index trotz Filter durch-
        #      rutscht (race waehrend des Filterns), ueberspringt FFmpeg ihn
        #      statt abzubrechen
        current_audio_indices = {s.get('index') for s in self.audio_streams}

        selected_audio_streams_to_map = []
        for cb, stream_data in self.audio_track_checkboxes:
            if cb.isChecked():
                if stream_data.get('index') in current_audio_indices:
                    selected_audio_streams_to_map.append(stream_data)
                elif log:
                    print(f"⚠️  Audio stream {stream_data.get('index')} not in current "
                          f"source - skipping (stale checkbox from previous batch file)")
        ...
        if selected_subtitle_stream is not None:
            # Re-validate the selection against the CURRENT source's subtitle streams.
            #
            # Hintergrund: Im Batch-Modus laeuft ffprobe asynchron - process_next
            # ruft process_single_file aber synchron sofort danach auf. Dadurch
            # kann die subtitle_combo noch die Stream-Daten der VORHERIGEN Datei
            # enthalten. Im aktuellen File hat derselbe Index unter Umstaenden
            # einen anderen Codec (PGS statt SSA o.ae.) - oder existiert gar nicht.
            #
            # Ohne diese Validierung baut der Build naiv `-map 0:N -c:s copy`,
            # und FFmpeg kracht mit "Subtitle encoding currently only possible
            # from text to text or bitmap to bitmap", weil der Matroska-Muxer
            # auf seinen text-basierten Default-Encoder (SSA) zurueckfaellt
            # wenn -c:s copy intern nicht durchkommt.
            sub_idx = selected_subtitle_stream.get('index')
            current_sub = next(
                (s for s in self.subtitle_streams if s.get('index') == sub_idx),
                None
            )
            sub_codec = (current_sub.get('codec_name', '').lower()
                         if current_sub else
                         selected_subtitle_stream.get('codec_name', '').lower())

            if current_sub is None:
                if log:
                    print(f"⚠️  Subtitle skipped: stream {sub_idx} not found in current source")
            elif self.burn_in_checkbox.isChecked():
                # Burn-in nutzt den libass-basierten subtitles-Filter. Picture-
                # basierte Subs (PGS/DVB/DVD/xsub) gehen damit NICHT - die
                # brauchten einen overlay-Filter mit dekodierten Frames.
                if sub_codec in self.SUB_PICTURE_BASED:
                    if log:
                        print(f"⚠️  Burn-in skipped: {sub_codec} is picture-based "
                              f"(libass needs text-based subs)")
                else:
                    escaped_path = input_path.replace('\\', '/').replace(':', '\\:')
                    video_filters.append(f"subtitles='{escaped_path}':si={sub_idx}")
            else:
                # Optional-Map (`?`) macht das Mapping robust: existiert der
                # Stream im aktuellen File nicht, ueberspringt FFmpeg ihn
                # statt abzubrechen. Schuetzt vor Stale-Index in Batch-Modus.
                maps.extend(["-map", f"0:{sub_idx}?"])
    ...
    def update_subtitle_combo(self):
        ...
        for i, stream in enumerate(self.subtitle_streams):
            ...
            # Visuelle Markierung: Bitmap-Subs sind im Convert-Workflow
            # potenziell problematisch (Burn-in geht gar nicht, -c:s copy
            # kann bei kaputten Codec-Metadaten den Encode killen).
            # Auswahl bleibt erlaubt - die Build-Validierung greift dann.
            if codec in self.SUB_PICTURE_BASED:
                marker = " ⚠ picture-based"
            elif codec in self.SUB_CONVERTIBLE:
                marker = ""
            else:
                marker = " (?)"

            item_text = f"Stream {stream.get('index', i)} ({lang.upper()}: {title}) [{codec}]{marker}"
    ...
    def _check_av1_mediathek_compatibility(self) -> bool:
        ...
        v3.0: Erfasst alle AV1-Hardware-Encoder (NVENC, AMF, QSV) und
        SVT-AV1, weil das Black-Screen-Problem auf der Decoder-Seite der
        Mediathek-Player liegt - vendor-unabhaengig. Beim Auto-Switch
        wird zum HEVC-Pendant des aktuell gewaehlten Vendors gewechselt
        (av1_amf -> hevc_amf, av1_qsv -> hevc_qsv, av1_nvenc/libsvt-av1
        -> hevc_nvenc oder libx265).

        Bietet automatischen Wechsel zu HEVC an oder Force AV1 Option.
        ...
        # Vendor-passendes HEVC-Pendant ermitteln
        av1_to_hevc = {
        ...
        suggested_hevc = av1_to_hevc.get(video_codec, 'hevc_nvenc')
        suggested_label = self.VIDEO_CODEC_LABELS.get(suggested_hevc, suggested_hevc)
        current_label = self.VIDEO_CODEC_LABELS.get(video_codec, video_codec)
        ...
        reply = QMessageBox.warning(
            ...
            f"<code>{current_label}</code> has known compatibility issues with Mediathek videos.<br>"
            "H.264 and HEVC work reliably with Mediathek-Safe mode.<br><br>"
            f"<b>Recommendation:</b> Switch to <code>{suggested_label}</code> for reliable results.<br><br>"
            "Choose an option:<br>"
            f"• <b>Yes</b> - Switch to {suggested_label} (recommended)<br>"
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Auto-Switch zum vendor-passenden HEVC
            for i in range(self.video_codec_combo.count()):
                if self.video_codec_combo.itemData(i) == suggested_hevc:
                    self.video_codec_combo.setCurrentIndex(i)
                    print(f"🔄 Auto-switched from {video_codec} to {suggested_hevc} for Mediathek compatibility")
                    QMessageBox.information(
                        self,
                        "Codec Switched",
                        f"Video codec changed to <code>{suggested_label}</code>.<br>"
                        "You can now proceed with encoding."
                    )
                    return True

            # Fallback: HEVC-Pendant nicht verfuegbar (z.B. weil Vendor-
            # Filter den ausgeblendet hat). User muss manuell wechseln.
            QMessageBox.warning(
                self,
                "HEVC Not Available",
                f"<code>{suggested_label}</code> is not available with the "
                "current GPU selection.<br>"
                "Please select a different codec manually."
            )
            return False
    ...
    def _probe_duration_sync(self, filepath: str, timeout: float = 5.0) -> float:
        """
        Synchroner Quick-ffprobe nur fuer die Container-Duration.
        Returns 0.0 bei jeglichem Fehler (Timeout, missing ffprobe, parse-Fail).

        Use case: process_single_file im Batch-Modus, wenn der async
        VideoInfoWorker noch nicht durch ist oder seinerseits 0 zurueck-
        gegeben hat. Statt den File hart zu killen, holen wir die Duration
        synchron nach. 5s Timeout reicht auch fuer NAS-Sources mit langem
        Container-Scan.

        Bewusst minimaler Subprocess-Call - nur format=duration,
        kein Stream-Probe (das wuerde deutlich laenger dauern).
        """
        try:
            kwargs = {
                "capture_output": True,
            ...
            if os.name == "nt":
                # Verhindert Console-Flicker und Doppel-Window beim Nuitka-Build
    ...
    def process_single_file(self, input_path, output_path, is_batch=False, cmd_override=None):
        ...
        if self.total_duration <= 0:
            # Im Batch-Modus: synchroner ffprobe-Fallback bevor wir aufgeben.
            #
            # Der async ffprobe-Worker laeuft moeglicherweise noch (Stale-Daten-
            # Race aus process_next), oder hat fehlerhaft total_duration=0 zurueck-
            # gegeben. Statt den File hart als Batch-Error zu killen, holen wir
            # die Duration mit einem 5s-Timeout-Sync-Probe nach. Wenn das klappt,
            # geht der Encode regulaer weiter.
            if is_batch:
                recovered = self._probe_duration_sync(input_path, timeout=5.0)
                if recovered > 0:
                    print(f"⚠️  Duration recovered via sync ffprobe for "
                          f"{os.path.basename(input_path)}: {recovered:.2f}s")
                    self.total_duration = recovered
    ...
    def _log_batch_error(
        ...
        """
        Schreibt einen Encoding-Fehler in eine Message.log
        im Verzeichnis der fehlgeschlagenen Eingabedatei.

        Im Gegensatz zu _log_batch_skip ist das ein "harter" Fehler:
        FFmpeg ist mit einem Returncode != 0 abgebrochen. Wir wollen den
        Batch trotzdem nicht killen — User soll am Ende ueber alle Files
        durch sein und in den Logs nachschauen koennen, welche Files es
        nicht geschafft haben.

        Beispielzeile:
            [2026-05-09 14:32:11] ERROR
                Input:    Magnum - S02E08.mkv
                Output:   Magnum - S02E08.AV1.mkv
                RC:       -22
                FFmpeg:
                    [sost#0:3/ssa @ ...] Subtitle encoding currently...
        ...
        # FFmpeg-Output kann hunderte Zeilen lang sein (z.B. Frame-Spam vor
        # dem Crash). Wir kuerzen auf die letzten ~30 Zeilen — das ist da,
        # wo die echte Error-Message sitzt. Jede Zeile mit 8 Spaces
        # eingerueckt, damit der Block visuell unter "FFmpeg:" sitzt.
        if isinstance(ffmpeg_output, str) and ffmpeg_output:
            tail_lines = ffmpeg_output.strip().splitlines()[-30:]
            indented = "\n".join("        " + line for line in tail_lines)
        else:
            indented = "        (no output captured)"
        ...
        try:
            ...
            # Log-Schreiben darf das Encoding nie blockieren
            print(f"⚠️ Could not write error log to {log_path}: {e}")
    ...
    def ffmpeg_finished(self, return_code: int, output: str):
        ...
        if self.batch_mode:
            ...
            # Subtitle-Encoding-Fehler? → einmaliger Retry ohne Subtitle.
            #
            # Wenn FFmpeg an "Subtitle encoding currently only possible from
            # text to text or bitmap to bitmap" scheitert, hat -c:s copy den
            # Stream nicht durchgekriegt (kaputte Codec-Metadaten, Bitmap-Sub
            # mit Text-Default-Encoder im Matroska-Muxer, Stale-Index nach
            # async ffprobe). Statt den ganzen Batch zu killen: einmal ohne
            # Subtitle retryen und im Skip-Log dokumentieren.
            #
            # _subtitle_retry_active verhindert eine Endlosschleife, falls
            # der Retry SELBST nochmal mit irgendeinem Subtitle-Fehler kracht
            # (dann wird der File regulaer als Fehler behandelt).
            if (return_code != 0 and not is_cancelled
                    and not getattr(self, '_subtitle_retry_active', False)
                    and isinstance(output, str)
                    and "Subtitle encoding currently only possible" in output):
                print(f"⚠️  Subtitle copy failed for {current_file} - "
                      f"retrying without subtitle")
                self._subtitle_retry_active = True

                # Combo temporaer auf "None" - der Build sieht im Retry kein
                # Subtitle und mappt nichts. Original-Index danach wieder-
                # herstellen, damit nachfolgende Files weiter den User-
                # Wunsch respektieren.
                prev_idx = self.subtitle_combo.currentIndex()
                self.subtitle_combo.setCurrentIndex(0)

                input_path = self.batch_files[self.batch_index]
                output_path = self.output_line.text()

                self._log_batch_skip(
                    input_path, output_path,
                    "Subtitle stream incompatible — retried without subtitle"
                )

                try:
                    self.process_single_file(input_path, output_path, is_batch=True)
                finally:
                    # Index restauriert, Flag bleibt aktiv bis ffmpeg_finished
                    # vom Retry-Run zurueckkommt - dann clearen wir es dort.
                    self.subtitle_combo.setCurrentIndex(prev_idx)
                return

            # Retry beendet (egal ob erfolgreich oder weiter fehlerhaft) →
            # Flag clearen, damit der naechste File wieder normal laeuft.
            if getattr(self, '_subtitle_retry_active', False):
                self._subtitle_retry_active = False

            # v3.0.3: Kein blockierendes QMessageBox mehr.
            #
            # Vorher zeigte ein QMessageBox.critical(...) den Fehler an und
            # wartete auf Klick — der gesamte Batch hing dadurch fest, bis
            # der User OK drueckte. Bei Over-Night-Batches mit ein paar
            # broken Files war das eine Katastrophe (am Morgen war erst
            # ein File durch und alle anderen warteten auf einen Click,
            # den niemand gemacht hat).
            #
            # Stattdessen: Console-Print + Eintrag in Message.log neben dem
            # Input-File + Counter-Inkrement fuer die Endbilanz im Toast.
            # Der Batch laeuft ohne User-Interaktion durch.
            if return_code != 0 and not is_cancelled:
                input_path = self.batch_files[self.batch_index]
                output_path = self.output_line.text()
                print(f"❌ FFmpeg error on {current_file} (RC={return_code}) — "
                      f"logged to Message.log, batch continues")
                self._log_batch_error(input_path, output_path, return_code, output)
    ...
    def show_batch_complete_toast(self, total_files: int) -> None:
        ...
        encoded = total_files - skipped - failed

        # v3.0.3: Drei Buckets statt zwei (encoded/skipped/failed). Failed
        # wird visuell hervorgehoben mit ⚠️, damit der User nicht uebersieht
        # dass ein paar Files NICHT erfolgreich durch sind und in
        # Message.log nachgeschaut werden sollte.
        if skipped == 0 and failed == 0:
            text = (
                f"✅ All {total_files} file{'s' if total_files != 1 else ''} "
                f"encoded successfully!"
            )
        elif encoded == 0 and failed == 0:
            text = (
            ...
            # Mindestens ein Encoding-Fehler — Toast macht das deutlich.
            lines = [f"⚠️ Batch complete with errors:"]
            if encoded > 0:
                lines.append(f"   • {encoded} encoded")
            if skipped > 0:
                lines.append(f"   • {skipped} skipped")
            lines.append(f"   • {failed} failed (see Message.log files)")
            text = "\n".join(lines)
    ...
    def show_about_dialog(self):
        ...
        # Zeilen (z.B. "Hardware Encoding ... NVIDIA NVENC, AMD AMF and
        # Intel QSV", "Strict VBV ... bufsize = 2x maxrate", "Mediathek-
        # Safe ... black-screen issues on Mediathek downloads") nicht rechts
        # abgeschnitten werden, und der Footer (Copyright + Lizenz) komplett
        # sichtbar bleibt.
        dlg.resize(820, 680)
        ...
        highlights = (
            item("🚀 Hardware Encoding",
                 "AV1, H.265, H.264 on NVIDIA NVENC, AMD AMF and Intel QSV "
                 "with multi-GPU support "
                 "(modern <code>-init_hw_device</code> backends)") +
        ...
        acks = (
            ...
            item("AMD AMF",             "hardware encoder") +
            item("Intel Quick Sync",    "hardware encoder") +
        ...
        html = (
            "<h3 style='margin: 0 0 4px 0;'>FFmpeg Converter GUI v3.0</h3>"
            ...
            f"(c) S. Friedrich 2025-2026 &middot; Version 3.0 &middot; "
            f"{time.strftime('%Y-%m')}"
            f"</p>"
